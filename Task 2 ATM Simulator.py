...
class ATM:                                  #(Declaring Class)
    def __init__(self, balance=0):          #(Declaring Function)
        self.balance = balance

    def check_balance(self):                #(Declaring balance cheking function)
        return self.balance

    def deposit(self, amount):              #(Declaring deposit function)
        if amount > 0:                      #(Using If loop)
            self.balance += amount
